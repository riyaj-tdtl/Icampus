...
import { Box, Typography, Button, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Chip, useMediaQuery, useTheme, Tabs, Tab } from '@mui/material';
...
const AttendancePage = () => {
  ...
  const availableTabs = isStudent
    ? []
    : isAdmin
      ? [{ label: 'Teacher Attendance', type: 'teacher', count: teacherRecords.length }]
      : [{ label: 'Student Attendance', type: 'student', count: studentRecords.length }];
  ...
  const handleMarkAttendance = () => {
    ...
    if (!targetOptions.length) {
      showError('Options Unavailable', `Could not load ${targetLabel.toLowerCase()} list. Please try again.`);
      return;
    }

    showComplexForm(`Mark ${targetLabel} Attendance`, [
      { id: target, label: `Select ${targetLabel}`, type: 'select', options: targetOptions },
      ...
      if (res.isConfirmed && res.value) {
        try {
          const selectedPerson = targetOptions.find((option) => String(option.value) === String(res.value[target]));
          const payload = {
          ...
          showSuccess('Attendance Marked!', `Attendance for ${selectedPerson?.label || `${targetLabel.toLowerCase()} ID ${res.value[target]}`} has been recorded.`);
          fetchAttendance();
        } catch (err) {
          showError('Failed', err.response?.data?.detail || 'Could not mark attendance');
        }
      }
    });
  };
  ...
  return (
    <Box sx={{ pb: 4 }}>
      <Box sx={{ 
        ...
        {(isAdmin || isTeacher) && (
          <Button 
            ...
            sx={{ borderRadius: '12px', textTransform: 'none', fontWeight: 700, py: 1.5, px: 3, bgcolor: '#8456f1', boxShadow: '0 8px 20px rgba(132, 86, 241, 0.3)', '&:hover': { bgcolor: '#7344e3' } }}
      ...
      <TableContainer component={Paper} sx={{ borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', overflowX: 'auto', border: '1px solid #f1f5f9' }}>
        <Table sx={{ minWidth: 650 }}>
            ...
            <TableRow>
              {isStudent ? (
                ...
                <>
                  ...
                  <TableCell sx={{ fontWeight: 800, color: '#64748b', fontSize: '0.75rem', textTransform: 'uppercase' }}>{isShowingStudents ? 'Student Name' : 'Teacher Name'}</TableCell>
                  <TableCell sx={{ fontWeight: 800, color: '#64748b', fontSize: '0.75rem', textTransform: 'uppercase' }}>Date</TableCell>
                  <TableCell sx={{ fontWeight: 800, color: '#64748b', fontSize: '0.75rem', textTransform: 'uppercase' }}>Status</TableCell>
                  {isShowingStudents && <TableCell sx={{ fontWeight: 800, color: '#64748b', fontSize: '0.75rem', textTransform: 'uppercase' }}>Marked By</TableCell>}
                  <TableCell sx={{ fontWeight: 800, color: '#64748b', fontSize: '0.75rem', textTransform: 'uppercase' }}>Recorded At</TableCell>
                </>
              )}
            </TableRow>
          </TableHead>
          <TableBody>
            {loading ? (
              <TableSkeleton cols={isStudent ? 2 : (isShowingStudents ? 6 : 5)} rows={5} />
                  ...
                  {isStudent ? (
                    <>
                      ...
                      <TableCell>
                        <Chip 
                          ...
                          sx={{ fontWeight: 700, minWidth: '85px', borderRadius: '999px', textTransform: 'none', px: 1.5 }}
                        />
                      </TableCell>
                    </>
                  ) : (
                    <>
                      ...
                      <TableCell sx={{ fontWeight: 700, color: '#0f172a' }}>
                        {isShowingStudents ? row.student_name : row.teacher_name}
                      </TableCell>
                      ...
                      {isShowingStudents && (
                        <TableCell sx={{ color: '#64748b' }}>#{row.marked_by}</TableCell>
                      )}
              ...
              <TableRow><TableCell colSpan={isStudent ? 2 : (isShowingStudents ? 6 : 5)} align="center" sx={{ py: 6, color: 'text.secondary' }}>No attendance records found.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </TableContainer>
